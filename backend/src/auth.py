...
baselink = "http://127.0.0.1:3000/otpverify" 
...
def login_phone():
    ...
    if user:
        if check_password_hash(user.password, password):
            if user.otp != "none":
                return jsonify({'error': 'Please verify your email first'}), 400
            access_token = create_access_token(identity={'id': user.id, 'phone': user.phone})
            user.logined = "true"
            db.session.commit()
            return jsonify({'message': 'Logged in successfully!', 'access_token': access_token}), 200
        else:
...
def sign_up():
    ...
    if user:
        ...
        new_user = User(email=email, name=name, password=generate_password_hash(
            password1, method='sha256'), websocket_id=uuid4().hex, profilepic=pic, otp=otpgenerator(name, email), phone=name)
